...
func (jq *JobQuery) QuerySkill() *SkillQuery {
...
//  WithSkill tells the query-builder to eager-loads the nodes that are connected to
// the "skill" edge. The optional arguments used to configure the query builder of the edge.
func (jq *JobQuery) WithSkill(opts ...func(*SkillQuery)) *JobQuery {
	query := &SkillQuery{config: jq.config}
...
func (jq *JobQuery) sqlAll(ctx context.Context) ([]*Job, error) {
	...
	if query := jq.withSkill; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Job)
		for i := range nodes {
			if fk := nodes[i].job_skill; fk != nil {
				ids = append(ids, *fk)
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "job_skill" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Skill = n
			}
		}
	}
